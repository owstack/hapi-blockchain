...
class Blockchain extends events.EventEmitter {
    constructor(options = {currency: 'BTC', nodes: []}) {
        super();
    ...
    _initDefaults(options) {
        const DEFAULT_MAX_TXIDS = 1000;
        const DEFAULT_MAX_HISTORY = 50;
        const DEFAULT_SHUTDOWN_TIMEOUT = 15000;
        const DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
        const DEFAULT_MAX_ADDRESSES_QUERY = 10000;
        const DEFAULT_TRY_ALL_INTERVAL = 1000;
        const DEFAULT_START_RETRY_INTERVAL = 5000;
        const DEFAULT_TRANSACTION_CONCURRENCY = 5;

        // limits
        this.maxTxids = options.maxTxids || DEFAULT_MAX_TXIDS;
        this.maxTransactionHistory = options.maxTransactionHistory || DEFAULT_MAX_HISTORY;
        this.maxAddressesQuery = options.maxAddressesQuery || DEFAULT_MAX_ADDRESSES_QUERY;
        this.shutdownTimeout = options.shutdownTimeout || DEFAULT_SHUTDOWN_TIMEOUT;

        // try all interval
        this.tryAllInterval = options.tryAllInterval || DEFAULT_TRY_ALL_INTERVAL;
        this.startRetryInterval = options.startRetryInterval || DEFAULT_START_RETRY_INTERVAL;

        // rpc limits
        this.transactionConcurrency = options.transactionConcurrency || DEFAULT_TRANSACTION_CONCURRENCY;

        // sync progress level when zmq subscribes to events
        this.zmqSubscribeProgress = options.zmqSubscribeProgress || DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
    }
    ...
    _rapidProtectedUpdateTip(node, message) {
    ...
    _getAddressesFromTransaction(transaction) {
        const addresses = [];
    ...
    _notifyAddressTxidSubscribers(txid, transaction) {
        const addresses = this._getAddressesFromTransaction(transaction);
        for (let i = 0; i < addresses.length; i++) {
            const address = addresses[i];
            this.emit('address', {address, txid});
        }
    }

    _zmqTransactionHandler(node, message) {
    ...
    async _checkSyncedAndSubscribeZmqEvents(node) {
        const DEFAULT_TIP_UPDATE_INTERVAL = 15000;
        ...
        if (!synced) {
            ...
            }, node._tipUpdateInterval || DEFAULT_TIP_UPDATE_INTERVAL);
        }
    }
    ...
    _normalizeAddressArg(addressArg) {
     ...
     */
    async getAddressBalance(addressArg) {
     ...
     * @param {Object} options
     */
    async getAddressUnspentOutputs(addressArg, options) {
    ...
    _getHeightRangeQuery(options, clone) {
     ...
     * @param {Object} options
     */
    async getAddressTxids(addressArg, options) {
    ...
    _getConfirmationsDetail(transaction) {
    ...
    _getAddressDetailsForInput(input, inputIndex, result, addressStrings) {
        if (!input.address) {
            return;
        }
    ...
    _getAddressDetailsForOutput(output, outputIndex, result, addressStrings) {
        if (!output.address) {
            return;
        }
    ...
    _getAddressDetailsForTransaction(transaction, addressStrings) {
        const result = {
            addresses: {},
            satoshis: 0
        };
     ...
     */
    async _getAddressDetailedTransaction(txid, options) {
        let transaction;
        try {
    ...
    _paginateTxids(fullTxids, fromArg, toArg) {
     ...
     * @param {Object} options
     */
    async getAddressHistory(addressArg, options = {}) {
        const addresses = this._normalizeAddressArg(addressArg);
     ...
     * @param {Object} options
     */
    async getAddressSummary(addressArg, options = {}) {
        const summary = {};
        const queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
        let summaryTxids = [];
        let mempoolTxids = [];
    ...
    async _maybeGetBlockHash(blockArg) {
     ...
     * @param {String|Number} block - A block hash or block height number
     */
    async getBlockOverview(blockArg) {
        try {
            const blockhash = await this._maybeGetBlockHash(blockArg);
     ...
     */
    async getBlockHashesByTimestamp(high, low, options = {}) {
        try {
     ...
     */
    async getDetailedTransaction(txid) {
    ...
    async getInfo() {
        try {
            ...
            const combinedInfo = {
                ...
                testnet: blockchaininfo.chain === 'testnet' ? true : false,
